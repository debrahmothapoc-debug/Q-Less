...
function Skel() {
  return (
    <div style={{background:"white",borderRadius:14,padding:16,marginBottom:12,boxShadow:"0 4px 24px rgba(26,58,143,0.12)",display:"flex",flexDirection:"column",gap:9}}>
...
export default function App() {
  ...
  if (user && user.role === "driver") {
    return <DriverApp initialUser={user} onLogout={() => { api.setToken(null); setUser(null); }} />;
  }
    ...
    <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",background:"#f5f7ff",fontFamily:"sans-serif",color:"#0f1b4c"}}>
      ...
      <nav style={{background:bl,padding:"0 20px",display:"flex",alignItems:"center",justifyContent:"space-between",height:62,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 16px rgba(26,58,143,0.4)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5}}>
          ...
          <button onClick={() => user ? setView("rides") : setShowAuth(true)} style={{padding:"7px 13px",borderRadius:7,border:"none",cursor:"pointer",fontSize:13,fontWeight:500,background:view==="rides"?"white":"transparent",color:view==="rides"?bl:"rgba(255,255,255,0.65)"}}>
            My Rides {myB.length > 0 && <span style={{background:"#f59e0b",color:"white",borderRadius:20,fontSize:9,fontWeight:700,padding:"1px 5px",marginLeft:3}}>{myB.length}</span>}
          </button>
          {user
      ...
      {view === "book" && <>
        <div style={{background:"linear-gradient(135deg,#1a3a8f 0%,#2952c4 60%,#4a7bf7 100%)",padding:"36px 22px 48px"}}>
          <div style={{display:"inline-block",background:"rgba(255,255,255,0.15)",color:"rgba(255,255,255,0.9)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:16,padding:"3px 13px",fontSize:11,fontWeight:500,marginBottom:14}}>South African Taxi Booking</div>
