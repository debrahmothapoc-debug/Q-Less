...
try {
const { rows } = await db.query(`SELECT u.id, u.name, u.phone, u.is_active, d.id AS driver_id, d.taxi_plate, d.rating, r.from_place || ' to ' || r.to_place AS route_name FROM users u JOIN drivers d ON d.user_id = u.id LEFT JOIN routes r ON r.id = d.route_id WHERE u.role = 'driver' ORDER BY u.created_at DESC`);
res.json(rows);
} catch (err) { next(err); }
});

// POST /api/admin/drivers — create driver user + profile in one call
...
res.status(201).json({
  message: "Driver account created for " + name,
  user_id: userId,
...
// PATCH /api/admin/drivers/:id/deactivate
