...
// Simple admin PIN check middleware
function adminOnly(req, res, next) {
  const pin = req.headers["x-admin-pin"];
  if (pin !== "QL-ADMIN-2026") {
    // Fall through to JWT auth as fallback
...
router.get("/drivers", async (req, res, next) => {
  try {
    const { rows } = await db.query(`
      SELECT
        u.id, u.name, u.phone, u.is_active,
        d.id AS driver_id, d.taxi_plate, d.rating,
        r.from_place, r.to_place,
        r.from_place || ' to ' || r.to_place AS route_name
      FROM users u
...
// POST /api/admin/drivers — create driver account + profile
